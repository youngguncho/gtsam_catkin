...
int main(int argc, char **argv)
{
    ...
    // Set up ROS.
    ros::init(argc, argv, "Pose2SLAM");

    // Create a factor graph container
    NonlinearFactorGraph graph;

    // Add a piror on the first pose, setting it to the origin
    ...
    noiseModel::Diagonal::shared_ptr odomModel = noiseModel::Diagonal::Sigmas(Vector3(0.5, 0.5, 1));

    // Add Odometry factors
    // Create odometry (between) factors between consecutive poses
    // Rpbpt makes 90 deg right turn at x3 - x5
    graph.add(BetweenFactor<Pose2> (Symbol('x', 1), Symbol('x', 2), Pose2(5, 0, 0), odomModel));
    Pose2 p2 = Pose2(5, 0, -M_PI_2);
    graph.add(BetweenFactor<Pose2> (Symbol('x', 2), Symbol('x', 3), p2, odomModel));
    graph.add(BetweenFactor<Pose2> (Symbol('x', 3), Symbol('x', 4), p2, odomModel));
    graph.add(BetweenFactor<Pose2> (Symbol('x', 4), Symbol('x', 5), p2, odomModel));
    ...
    // Apply initial variable values for the optimization
    // Add random noise from ground truth value
    Values initials;
